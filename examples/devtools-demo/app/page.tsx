...
export default function HomePage() {
  const [lastResult, setLastResult] = useState<string>('Click a button — watch the devtools tabs light up.')
  ...
  async function callApiWithToken() {
    setLastResult('Calling /api/hello?token=eyJabc123 (token will appear [REDACTED] in devtools)...')
    try {
      ...
      const data = await res.json()
      setLastResult(`OK — ${JSON.stringify(data)} (check Requests tab — Authorization + ?token= are [REDACTED])`)
    } catch (err) {
      setLastResult(`ERROR — ${String(err)}`)
    }
  }
  ...
  return (
    <>
      ...
      <p>
        This page lives at <code>app/page.tsx</code>. The devtools <strong>Routes</strong> tab
        will highlight it. Click any link in the nav above to see the highlight follow you.
      </p>
      ...
      <h2>What to do</h2>
      <ol>
        <li>Click the floating chip bottom-right (it says <code>theo</code>).</li>
        <li>Open <strong>Requests</strong> tab → click any button above → row appears within 100 ms.</li>
        <li>Click a row to expand: method, path, status, duration, traceId, headers (redacted!).</li>
        <li>Open <strong>Errors</strong> tab → click "Raw fetch" or "console.error" — entries with <code>code</code> + clickable <code>docsUrl</code>.</li>
        <li>Open <strong>Routes</strong> tab → see the tree of <code>app/**</code>; click another link in nav → leaf highlight follows.</li>
        <li>Open <strong>Settings</strong> tab → change position or theme → reload — preserved via localStorage.</li>
        <li>Press <strong>Escape</strong> to close the panel; <strong>Ctrl+Shift+D</strong> to hide/show the chip.</li>
