...
This file complements the global engineering rules at `~/.claude/CLAUDE.md`, which apply unconditionally. This file adds TheoKit-specific layers on top and **does not** propagate to the sibling repos in the group.

**Layout (2026-08 reorg).** The former `usetheo/` monorepo was split into repo groups under `/home/paulo/Projetos/theo/`: `theokit-framework/` (this group — TheoKit, SDK, UI, plugins, studio, TUI, …), `theo-platform/` (the Go platform: `theo`, `theo-cloud`, `theo-lens`, …), `theo-business/` (website, docs, blog) and `usetheo-labs/` (experiments). There is **no monorepo-level `CLAUDE.md`** in this layout; the Locked Narrative that used to live in it survives as the inherited copy in `../../theo-business/theo-website/CLAUDE.md` § Locked Narrative, which stays authoritative for cross-product positioning.

**Where the knowledge lives (2026-08-06).** The former `docs/` and `knowledge-base/` trees were
merged into a single [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)
v0.2 bundle at [`wiki/`](wiki/index.md) — 73 concepts covering the product documentation *and*
the decision trail (grill → blueprint → plan → review → ADR → milestone). Start at
`wiki/index.md`; every concept carries `type` frontmatter and links to what it relates to. The
`.claude/knowledge-base/` tree (the plan ecosystem's working directory) is **separate and
untouched** — it is where in-flight cycle artifacts live, while `wiki/` is the published record.
`knowledge-base/references/` (the read-only peer-study zone) was removed with the rest; the
curated catalog of what was studied survives at `wiki/references-catalog.md`.

---

## How TheoKit connects to TheoCloud (read this BEFORE editing any seam code)

TheoKit (this repo) emits `.theokit/services.json` v2 — the **only** contract surface with the **TheoCloud commercial PaaS** (`../../theo-platform/theo`). The two systems are deliberately decoupled per the `TheoCloud-first re-lock (2026-05-27)` invariant — neither imports the other; the artifact on disk is the seam.

**Canonical integration doc:** `../../theo-platform/theo/docs/architecture/theokit-theocloud-integration.md`. It contains the end-to-end flow diagram, the wire-protocol contract, the typed-error cause chain, the services.json schema-version table, and the hardening invariants from Plan v1.2 (the 22 findings closed on the integration seam).
...
**Only the TheoCloud-side copy exists today** — the TheoKit-side mirror at `wiki/architecture/theokit-theocloud-integration.md` is **absent** (verified 2026-08-06). Until it is restored, read the TheoCloud copy; when it is restored, keep both in sync (edit one, copy the diff to the other in the same commit).
...
The **vocabulary table** and **before/after examples** are living — add new entries as new TheoKit copy lands or new patterns are discovered. The **communication layers**, **banned terms list**, and **storytelling rules** require an explicit strategic review (same gate as the Locked Narrative table in `theo-website/CLAUDE.md`). Do not weaken these to make a piece of marketing copy fit.
...
Plan: `.claude/knowledge-base/plans/dogfood-regressions-fix-plan.md` v1.1 — **no longer present** in `.claude/knowledge-base/plans/` (verified 2026-08-06); the old link also carried a stray `../` prefix.

---

## How this file relates to the repo group

- The global engineering rules at `~/.claude/CLAUDE.md` still apply inside `theokit/`. This file does not override them. (The former monorepo-level `CLAUDE.md` with "Cross-Project Rules 1–10" did not survive the 2026-08 repo-group split — see the layout note at the top of this file.)
- The voice and tone defined here is **scoped to the TheoKit project tree**. Do not export these rules to the sibling repos without an explicit strategic review at the group level.
- The Locked Narrative table in `../../theo-business/theo-website/CLAUDE.md` § Locked Narrative is authoritative for cross-product positioning (headline, sub-headline, comparison stack). TheoKit copy must not contradict it; TheoKit copy may be more aspirational *within* the TheoKit-shaped slice of that narrative.
...
|---------|--------------|------|-----------|----------------------------------------|-------------|:------:|
| **`@theokit/sdk`** + `@theokit/gateway` + `@theokit/gateway-telegram` | `../theokit-sdk/` (TypeScript) | Agent runtime: `Agent.getOrCreate/send`, `Run.stream`, providers (OpenAI/Anthropic/Ollama/OpenRouter), native `.jsonl` session transcript (SE40, SDK-owned persistence), custom-tool runtime | **TheoKit ← sibling** (TheoKit consumes it) | **npm registry (optional peer) — NOT a workspace link.** Consumed via the published `@theokit/sdk` as an optional `peerDependency` (`^4.0.1` in `packages/{theo,agents}`); loaded via dynamic `import()` so api-only apps need no SDK. `pnpm-workspace.yaml` does **NOT** include `../theokit-sdk/` — the sibling links were **removed 2026-06-10** (Turborepo requires workspace packages inside the repo root; SDK consumed via npm registry instead). Local sibling edits require a publish (or a manual `pnpm link`), NOT automatic reflection. The seam is drift-guaranteed by the M48 contract test (consumer + producer), the `CustomTool` type gate, and the boot-time version fail-fast — see [`wiki/architecture/theokit-sdk-integration.md`](wiki/architecture/theokit-sdk-integration.md). | Bridge `packages/agents/src/bridge/*` (`sdk-adapter` dynamic `loadSdkRuntime`), `packages/theo/src/server/define/define-agent-tool.ts` (`CustomTool` mirror), examples + templates `from '@theokit/sdk'`. | ✅ Wired (npm optional peer + M48 seam guarantees) |
| **`@theokit/ui`** | `../theokit-ui/` (TypeScript) | AI-native React component library for AI-agent surfaces (coding agents + chat): chat surface (`ChatMessage`, `ChatThread`, `ChatComposer`, `ToolCallCard`, `AgentEvent`, `DiffViewer`, `BuildLogStream`), theme system (`ThemeProvider`, `ThemeScript`, `TheoUIProvider`), design tokens; 99 components. The generic primitives + cloud-ops composites moved to `@usetheo/ui` (AI-exclusive pivot 2026-07-03), which `@theokit/ui` depends on. | **TheoKit ← sibling** (TheoKit consumes it) | **npm dep + optional peerDep** — published `@theokit/ui` (currently `1.0.0`) consumed via the npm registry; declared como **optional `peerDependency`** em `packages/theo/package.json` (ADR [0018](.claude/knowledge-base/adrs/0018-usetheo-ui-vite-plugin-contract-versionado.md)). `pnpm-workspace.yaml` does **NOT** include `../theokit-ui/` por default — local edits no sibling NÃO refletem sem publish. **Opt-in** via `pnpm theo-ui:link` (ADR [0020](.claude/knowledge-base/adrs/0020-cross-repo-workspace-link-opt-in.md)) — copia `pnpm-workspace.linked-ui.yaml` sobre o canonical e linka o sibling para HMR cross-repo. Pre-commit hook GATE 0 bloqueia commit-com-link (force `--no-verify` proibido). CI sempre usa canonical (preserva publish-and-bump path). | Framework auto-injects `<TheoUIProvider>` in SSR + client entries when the package is detected. **Contract test cross-repo** em `tests/integration/contract-usetheo-ui-vite-plugin.test.ts` (consumer) + `theokit-ui/tests/contract/theokit-consumer.test.ts` (producer, gated por `prepublishOnly`). 13 files reference it: `vite-plugin/{inject-stylesheets,integrate-ui,theoui-detect,config-resolve,auto-detect,auto-detect-types,index}.ts`, `router/entry{,-server}.ts`, `config/schema.ts`, `cli/commands/{dev,upgrade-readiness}.ts`, `server/cost/track-agent-run.ts`. | ✅ Wired (npm + optional peer + opt-in link) |
| **`theo` → TheoCloud** (formerly "Theo PaaS") | `../../theo-platform/theo/` (Go) | **The principal deploy target** — hosted product where TheoKit apps run in production. K8s operators (Crossplane-style), Helm charts, multi-tenant control plane, managed Postgres + Redis, secret rotation, audit log persistence, distributed rate-limiter store. Separate Go CLI named `theo`. | **TheoKit → sibling** (TheoKit deploys to it) | **Adapter shipped Wave 3 v2.0 (thin validator)** — `packages/theo/src/adapters/theo-cloud.ts` exists and `theokit build --target theo-cloud` exits 0. The OSS adapter is intentionally thin: it validates `.theokit/services.json` (existing zod gate), logs the services that will be deployed, and returns. **K8s manifest emission lives ENTIRELY inside TheoCloud (proprietary Go code)** upon receiving the upload — TheoKit OSS never emits K8s YAML. Rationale (architectural decision 2026-06-05): OSS framework MUST emit formats consumed by public/open systems OR neutral exchange formats; NÃO formats consumed by proprietary closed systems. Exposing K8s shape in OSS would leak TheoCloud-internal infrastructure choices. Architectural hooks still in place: `JobBackend` interface (ADR-0002), `UsageStorageAdapter` interface (R0.5.11 design), `RateLimitStorageAdapter` (security-hardening plan), structured logging to stdout. | TheoCloud-side issues #58, #59, #60 interlock with TheoKit's security primitives. Next: TheoCloud Go-side cutover plan (Phase B) consumes the uploaded services.json bundle and emits K8s internally. | ✅ **Primary target — thin OSS adapter shipped, K8s emission TheoCloud-internal** |
| **`theokit-plugins`** — first-party plugin registry | `../theokit-plugins/` (TypeScript) | Container repo for official Fastify-style plugins that consume TheoKit's `TheoPlugin` SDK. **Today (verified 2026-08-06): 11 packages shipping** — `@theokit/auth-github` 0.1.2, `@theokit/auth-google` 0.1.2, `@theokit/auth-magic-link` 0.2.1, `@theokit/plugin-canvas` 0.3.3, `@theokit/plugin-copilot` 0.1.2, `@theokit/plugin-db-drizzle` 0.1.2, `@theokit/plugin-email` 0.1.2, `@theokit/plugin-forms` 0.1.4, `@theokit/plugin-payments` 0.2.1, `@theokit/plugin-realtime` 0.1.2, `@theokit/plugin-voice` 0.7.3. **`@theokit/plugin-cors` does not exist in the repo** — the earlier "1 package shipping (plugin-cors)" claim, and the `plugin-sentry` / `plugin-i18n` proposals gated on a "cors release", are stale. | **TheoKit → sibling** (sibling consumes TheoKit — direction INVERTED) | **Zero code wiring in framework core.** TheoKit does not import, dynamic-resolve, or auto-load anything from `theokit-plugins`. The sibling consumes TheoKit via npm `peerDependencies` + the `TheoPlugin { name, register(app) }` interface re-exported from `theokit/server`. Apps install plugins explicitly (`pnpm add @theokit/plugin-cors`) and pass them to `defineConfig({ plugins: [...] })`. | **Anchors live here, not there:** `ADR-0011` (strategy + temporal gates — **absent**, verified 2026-08-06); `docs/concepts/plugins.md` §7 (authoring guide — **absent**, verified 2026-08-06); `ADR-0008` (the SDK they consume — **absent**: the surviving ADR set starts at 0018 in `.claude/knowledge-base/adrs/`). | 🌱 **Sibling — 11 plugins shipping (auth × 3, plugin × 8)** |
| **`theo-stacks` → `create-theo`** (being absorbed into `create-theokit`) | **no local checkout** — the `theo-stacks/` repo is not present anywhere under `/home/paulo/Projetos/` as of the 2026-08 reorg (verified 2026-08-06); treat this row as historical context for the absorption, not as a path you can `cd` into (TypeScript) | Standalone polyglot scaffolder published as `create-theo` on npm. Today ships **19 templates in 7 languages** (Node · Go · Python · Rust · Java · Ruby · PHP — `node-express`, `node-fastify`, `node-nestjs`, `go-api`, `python-fastapi`, `rust-axum`, `java-spring`, `ruby-sinatra`, `php-slim`, `node-nextjs`, `fullstack-nextjs`, 7 monorepo-* variants, `node-worker`) with health probes, graceful shutdown, Dockerfile, CI per template. | **TheoKit ← (absorbing) sibling** | **Decision 2026-05-27 (ADR-0013 in this repo, to be drafted):** TheoCreate's scaffolding role is folded into `create-theokit`. Wave 1: TS templates already in `packages/create-theo/templates/`. Wave 2: import `python-fastapi` + `node-fastify` (or `node-hono`) templates, adapt to live next to TheoKit as `services/*/`. Other 5 languages deferred to future ADRs with demand evidence. Standalone `theo-stacks` repo + `create-theo` npm package go into deprecation. | Templates to absorb: `python-fastapi` (priority), `node-hono` (to be added — new, replaces `node-fastify`/`node-express` for fetch-handler shape). **Template set narrowed to `default`-only on 2026-06-17 (ADR [0023](.claude/knowledge-base/adrs/0023-default-only-template-set.md)):** the canonical template is `default` (shipped by `create-theokit`); the `create-theo` extras (`api-only`/`dashboard`/`postgres`/`saas`) were removed. Polyglot backends come via the `--backend` flag on `create-theokit`, not separate templates. | 🟡 **Being absorbed — default-only (ADR 0023); standalone repo enters deprecation when absorption completes** |
...
4. **`@theokit/ui` is a published npm dep by default; workspace-link is OPT-IN** (ADR [0020](.claude/knowledge-base/adrs/0020-cross-repo-workspace-link-opt-in.md)). Cross-repo PR flow para evoluir junto: **(a)** `pnpm theo-ui:link` no `theokit/` para iterar com HMR (auto-checks `../theokit-ui/dist/vite-plugin.js`), **(b)** ship a mudança em `../theokit-ui/` + roda contract test local (`pnpm test:contract`), **(c)** `pnpm theo-ui:unlink`, **(d)** publish `theokit-ui` (now at stable `1.0.0`; gated por `prepublishOnly`), **(e)** bump TheoKit consumer (`packages/theo/package.json:peerDependencies['@theokit/ui']`), **(f)** `pnpm install` + `pnpm sync:templates` para propagar aos templates. **NÃO** ativar o link cross-repo em CI/release (preserva publish-and-bump validation path). Tampouco adicionar `theokit-ui/` ao `pnpm-workspace.yaml` canonical — isso destruiria o ciclo de release validado. Contract test cross-repo (consumer + producer) é o gate runtime; peerDep range é o gate install-time.
...
7. **`theokit-plugins` is a DOWNSTREAM sibling, not an upstream dependency.** The framework core ships zero coupling to it — no auto-load, no preset, no convention. Apps install individual plugins (`@theokit/plugin-forms`, `@theokit/plugin-payments` etc.) and wire them via `defineConfig({ plugins: [...] })`. The strategy that governs which plugins ship lives in TheoKit's ADR-0011, not in the sibling — because the gate (community demand) is observed from TheoKit core, where the `TheoPlugin` SDK lives. **Do not** add `theokit-plugins` to `pnpm-workspace.yaml` — the per-plugin `devDependency: "theokit": "link:../../../theokit/packages/theo"` is the local-dev link, by design.
...
- **Devtools surface — IMPLEMENTED 2026-05-19 (commit `e369f4a`).** After full prior-art audit of TanStack Router devtools, Next.js `next-devtools/dev-overlay`, and Astro `dev-toolbar` (other frameworks — Remix, SvelteKit, Nitro, Hono, tRPC — verified to ship no devtools surface of their own).
  ...
  - **Artifact:** [`.claude/knowledge-base/reference/devtools.md`](.claude/knowledge-base/reference/devtools.md) — 1163-line deep dive (TanStack/Next.js/Astro file:line citations, 29 edge cases, 7 convergent patterns, 7 divergent patterns with TheoKit choices). `docs/plans/devtools-plan.md` (**absent from the repo** — verified 2026-08-06) — 1860-line execution plan, 10 ADRs, 13 tasks, edge-case-plan reviewed. Anyone wanting to re-open these decisions reads both docs first.
...
- **Replacing Next.js for everyone.** TheoKit is a vertical framework for agent-shaped apps. The framing in the Locked Narrative (`../../theo-business/theo-website/CLAUDE.md`) — "the app the agent lives in" — is the wedge. Trying to be a horizontal Next replacement dilutes the wedge.
...
- **A parallel agent runtime / orchestration engine.** TheoKit ships the *home* for an agent, not the agent runtime. A second loop that calls LLMs, dispatches tools, or owns conversation storage is out of scope — the SDK (or Mastra / Vercel AI SDK) owns that. **Refined by ADR 0038 (M4, 2026-07-04):** wiring the SDK's own primitives + the framework's shipped decorators into the app harness — HITL approval gates, resume, checkpoints — **IS** in scope. That is the "home the agent lives in" (auth, sessions, human gates, deploy), the framework's wedge, not the SDK's job. The harness makes no LLM call, dispatches no tool, runs no second loop, and creates no new store — it drives the existing `runReflectiveLoop` (which wraps `run.stream()`) and persists via the SDK's `ConversationStorageAdapter`. `examples/*` remain patterns; HITL + checkpoint become framework surface because they are app concerns.
