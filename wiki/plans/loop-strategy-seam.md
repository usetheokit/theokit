---
type: Plan
title: Plan: open the LoopStrategy seam
description: Opening loopStrategy by composition and moving the termination ceiling into the runner.
tags: [plan, runner, ocp]
status: stable
generated: { by: theokit-agent/unrecorded, at: 2026-07-24T00:00:00Z }
migrated: { by: claude-opus-5/okf-skill, at: 2026-08-06T00:00:00Z, from: knowledge-base/plans/loop-strategy-seam-plan.md }
sources:
  - id: origin
    resource: knowledge-base/plans/loop-strategy-seam-plan.md
...
O grill (`grills/loop-strategy-seam.md`) mapeou a assimetria de OCP. A discovery (`blueprints/loop-strategy-seam.md`, `SHIPPABLE` 99.7) achou que abrir o seam expõe dois defeitos que hoje ficam escondidos porque o critério de parada é interno:
...
|---|---|---|
| Blueprint desta discovery | as 4 decisões (D1 teto no runner; D2 name string + zod intacto; D3 padrão .compaction; D4 re-resolução) | `blueprints/loop-strategy-seam.md` |
...
| Grill | a assimetria de OCP e os 2 riscos | `grills/loop-strategy-seam.md` |
...
- `decisions/0004-loop-strategy-seam.md` (novo)
...
REFACTOR: None expected
VERIFY:  grep -c "^### D" decisions/0004-loop-strategy-seam.md  → 4
```
...
#### Acceptance Criteria
- [ ] ADR contém D1..D4: `grep -c '^### D' decisions/0004-loop-strategy-seam.md` retorna 4 e `grep -c 'Alternativas consideradas' ...` retorna 4
