---
type: Backlog
title: Agent feature backlog
description: Living record of agent features identified during documentation review, against Mastra parity.
...
|---|---|---|---|
| Doc conceitual de overview | P2 | DONE | `agents/overview.md` criado |
...
| Agentes como tools (padrão supervisor) | P2 | DONE | `defineSubAgent` + `createSquad` em `@theokit/sdk/a2a`. Ver `agents/multi-agent.md`. |
...
| Gap | Severidade | Disposição | Notas |
|---|---|---|---|
| Sandbox de execução de código (`createCodeMode`) | P3 | **DONE (M29)** | `createCodeMode({ tools, sandbox, onPermissionRequest })` — API restrita (só tools declaradas) + gate de permissão obrigatório (sem default-allow); a isolação é um `sandbox` **injetado** (app fornece engine vetado — nunca `node:vm`). Threat model em `agents/code-mode.md`. `theokit@0.17.0`. |
...
|---|---|---|
| `agents/overview.md` | ✅ | Diagrama do pipeline + decisão agente vs action vs route |
| `agents/using-tools.md` | ✅ | `@theokit/sdk-tools` pré-prontos, `ctx.context` (M7), decorator `@Tool` |
| `agents/skills.md` | ✅ | SKILL.md filesystem-based, `discoverSkills` + `buildSkillsBlock` públicos, `agent.skills.list()` |
| `agents/memory.md` | ✅ | `ConversationStorageAdapter`, `MemorySettings` + dreaming sweep, semantic index |
| `agents/multi-agent.md` | ✅ | `defineSubAgent` + `createSquad`, delegation hooks (M12), tool scope, delegation depth |
| `agents/structured-output.md` | ✅ | `Agent.generateObject` + `Agent.streamObject`, `errorStrategy` (M14) |
| `agents/human-in-the-loop.md` | ✅ | `@HumanInTheLoop`, `defineAgent({ approvals })` (M14), `GET /approvals` |
| `agents/mcp.md` | ✅ | `@MCP` decorator + `POST /mcp` serving (M16), envPolicy seguro, OAuth 2.1 PKCE |
| `agents/guardrails.md` | ✅ | M9 — 5 detectores, input fail-fast + output buffer/moderate, guards custom |
| `agents/a2a.md` | ✅ | M15 — agent cards `/.well-known/`, `createA2ATool` client + auth |
| `agents/processors.md` | ✅ | M10 — `createToolHooksPlugin` tool + LLM-turn hooks, veto |
| `agents/acp.md` | ✅ | M17 — `createACPTool` + `AcpClient`, `onPermissionRequest` obrigatório |
| `agents/code-mode.md` | ✅ | M29 — `createCodeMode`, boundary de sandbox **injetado** + gate de permissão, threat model |
| `agents/channels.md` | ✅ | M27 — webhook routes por plataforma, validação de assinatura (Slack/Telegram/Discord Ed25519) |
| `agents/sdk-agents.md` | ✅ | M28 — `createVendorAgentTool`, vendor client injetado, resume via session id |
...
| `agents/using-tools` | `agents/using-tools.md` | ✅ |
| `agents/skills` | `agents/skills.md` | ✅ |
| `agents/agent-memory` | `agents/memory.md` | ✅ |
| `agents/supervisor-agents` | `agents/multi-agent.md` | ✅ |
| `agents/structured-output` | `agents/structured-output.md` | ✅ |
| `agents/agent-approval` | `agents/human-in-the-loop.md` | ✅ |
...
| `agents/code-mode` | Backlog P3 → **shipped M29** + `agents/code-mode.md` | ✅ |
...
| Página Mastra | Doc TheoKit gerado | Status |
|---|---|---|
| `mcp/overview` | `agents/mcp.md` → **serving shipped M16** | ✅ |
