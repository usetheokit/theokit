...
- **M14 HITL surface expansion — `@theokit/agents` + `theokit`:** (a) `defineAgent({ approvals: { <toolName>: { question, timeout?, onTimeout? } } })` gates a tool without the `@Agent` class + `@HumanInTheLoop` decorator — compiles into the same `compiled.hitl` map the decorator path produces (reuses the proven endpoint HITL wiring), failing fast when an approval names an undeclared tool. (b) **`GET /api/agents/<name>/approvals`** lists pending HITL approvals: the `ApprovalRegistry` now tracks pending metadata (`toolName`, `question`, `expiresAt`) and exposes `list()`; `handleListApprovals` serves it as JSON, wired into the dev middleware. 8 tests (3 approvals config + 5 listing); approve/mount/agent-handlers tests green (no regression); lint + tsc clean. `errorStrategy` on `generateObject` (SDK) + forwarding `toolName` through the plugin wiring are follow-ups. (M14)
