...
- **Zero-config agent surface — groundwork** (M2, Eixo B): `@theokit/agents` now exports `defineAgent({ input, model, system, tools })` — the canonical imperative agent surface a top-level `agents/<name>.ts` default-exports (ADR-B1); it is a pure normalizer (no LLM at define time) that lowers to the same SDK-ready shape the `@Agent` decorator produces, so both surfaces share one runtime. The framework build now scans a top-level `agents/` directory (`scanAgents`) and records agents in the manifest, so `agents/support.ts` → `/api/agents/support` is discoverable at build and load. The dev/prod endpoint mount + the end-to-end typed client binding land in the same milestone. Agents live in a top-level `agents/` (sibling of `server/`) per the LOCKED naming decision; the existing route/action/ws scanners still ignore `agents/`. (theokit-ai-first M2)
