...
- **The `theoclaw` CI job ran and could not block a merge, and three of the guard's own helpers answered the same way to two different questions.** Found by an independent audit of the change that added them (`loop-code-review`, 4 blocking findings). The job was absent from `ci-ok`'s `needs:` — `ci-ok` is `name: CI`, the one context branch protection requires, verified against the live remote — so its suite, its two typechecks and its boundary check reported their own status and nothing read it: advisory while looking mandatory. A guard in `tests/smoke/ci-workflow.test.ts` now fails on any job that cannot reach it, with `pr-report` declared as deliberate (`ci-ok` exists BECAUSE `pr-report` only runs on same-repo pull requests) and `theocode`/`env-verdicts` declared as debt older than the guard (#B-016). Separately, the widening of `testRoots()` from a hardcoded `packages/` walk to the workspace member list — the central fix of the change — was pinned by nothing: reverting it lost exactly ONE file from the sweep, 1039 to 1038, against a >400 floor with 638 files of slack, so every existing assertion stayed green on the reverted code. The new pin was PROVEN rather than asserted, by running it against a copy carrying the old `testRoots()`. And two silent-empty answers are now refusals: `expandGlob` threw nothing on a pnpm form it cannot read (`**`, a partial `theokit-*`, a `!negation` all expanded to `[]`, indistinguishable from an absent member — latent today, since all six declared globs are supported forms), and `readdirSafe`/`isDirectory` caught every error where their own comment named exactly one, so EACCES, ENOTDIR and EMFILE under a parallel run all reported "this member contributes no test roots" and the sweep went on reporting agreement with a claim it had measured against a smaller tree. (#B-011)
...
- **`pnpm -r run build` ordered `apps/theocode` before the packages it bundles, and now does not.** pnpm orders by dependencies between workspace PROJECTS; the declaration lived at `apps/theocode/packages/agent`, which has no `build` script, while the script sits at `apps/theocode`, whose manifest named no framework package. The bundle started alongside `packages/http` and produced 77 `Could not resolve "@theokit/agents/..."` errors against a `dist/` that did not exist yet. Two lines declaring what is already true; measured both directions — without them this project builds 1st and `packages/agents` 6th, with them 5th and 6th. A build-ordering edge is not an import, so `knip.jsonc` carries the reason rather than a silent exemption. Caught by `dep-check`, whose three expensive runs only turn on for a `main`-based pull request. (#B-111)

- **The root-bar orphan guard takes a full version instead of a major.** `sinceMajor` was built for the 4.x → 5.x transition and its own comment says so; seven new verdicts written as `sinceMajor: 5` claimed "present anywhere in the 5 line", which is false for every one of them. `@theokit/agents` declares `^5.3.0` and the floor run installs exactly that. Measured with the instrument the test uses — `Object.keys(await import('@theokit/sdk'))` across six versions — five arrived at **5.6.0** and two at **5.8.0**; not one is 5.9.0, which is what guessing from the bump that prompted the work would have produced. The `.d.ts` disagrees and would have given two wrong answers on its own. Of the 98 entries in the table, zero lack a `since` while being absent at the floor. (#B-111)
...
- **The `local` scope had never run through `applySubagentMemory` either, and now does.** The lower layer covered all three roots while the wrapper covered one; `local` shares `cwd` with `project` and differs only in the directory, and "it takes the same branch" is the reasoning that let the `user` rename live. The test asserts the committed note is NOT read, so a wrong directory name fails loudly instead of quietly serving the wrong file. (#B-091)
- **The README's surfaces table carried three claims the shipped code contradicts, and it is the copy that travels.** `.claude/rules/` is not versioned, so a correction written there reaches one machine; the README reaches everyone who installs the package. `~/.claude.json` personal-scope MCP servers still said **not read yet** after `loadPersonalMcpServers` shipped; `keybindings.json` and `themes/*.json` still named `@theokit/tui` as owner while the prose four paragraphs below said the toolkit reads neither — the document contradicted itself, and the row is the half a reader checks. Measured: `loadMcpJson`, the neighbour marked **read**, has no internal caller either, so both MCP rows are the same state and `agent-memory/` differs for a reason that is not about callers — its declaration was DROPPED by `@theokit/sdk` before a reader could see it. (#B-091)
- **The test guarding that table failed when a gap got CLOSED.** It asserted the literal string `not read yet` appears somewhere in the README, which held only while some surface was undone; closing the last one emptied the phrase of a referent and the assertion began demanding the document describe a gap the package no longer has. It now pins the property instead: the two `~/.claude.json` halves must carry DIFFERENT verdicts, and every row must OPEN with one of seven measured verdicts — `startsWith`, so `planned to be read later` is refused where a substring match would find `read` in it and pass. Both directions proved with positive controls. (#B-091)
- **The entry-point gate deadlocked every release that bumps a package the meta-package depends on.** It runs before the publish step, and `theokit`'s `@theokit/agents: workspace:^` packs to the version being released — which the registry does not have yet, by construction. `npm install` of that tarball failed and the release died before publishing anything. A `@theokit/*` dependency this run also packs is now installed from that tarball. The gate went from 32 entry points to **60**: the meta-package's 28 had never been probed, because it never installed. (#B-089)
- **The typecheck gate's budget reported a passing typecheck as a failing gate.** It timed out at 300s inside `check:all`; re-measured isolated the same day it takes **161s**, against the 97s the file itself recorded — the cost grew 66% while nobody was looking, which is the drift that file's own note predicted. Under the aggregate it spawns a full seven-package typecheck while a second one and two linters run beside it. Raised to 900s with the measurement written in, because the failure is asymmetric: the run reported `1 failed` beside `Type Errors: no errors`. (#B-110)

## [@theokit/agents 14.5.0] - 2026-09-16
...
- **Every declared entry point is now proved to LOAD from its tarball, not merely to exist in it.** `scripts/check-pack-entry-points-load.mjs` packs each publishable package, installs the archive into an empty directory, and imports all 60 non-wildcard subpaths the export maps declare. It catches what presence cannot: an undeclared runtime dependency, a module that throws on evaluation, a `files` list that shipped the file and not what the file imports. The gap was named by TheoCode itself, reviewing the sibling presence-gate through a frontier model and asked what class of broken publish it would miss — it named the case where the resolver finds `package/dist/index.js` and the error surfaces only when `import` actually runs (B-089). Two refusals in it are measured rather than cautious: a package declaring only `bin` promises a command and is skipped (probing `create-theokit` for an importable "." invented an entry point it never offered), and peers are installed before importing (`@theokit/agents/client/react` declares `react: >=18`, and probing without it accuses the package of breaking the contract it honours) — both surfaced as false failures on the first run, 2 of 61, neither a defect. Negative control: adding a subpath that points at no file makes it exit 1. It runs in `release.yml` rather than `check:all`, because it installs seven tarballs while the 2-second presence gate stays where it is. (B-089)
...
- **The `agent-memory/` row said `read` and claimed the applier runs; no consumer calls it.** The paragraph two sections below already said the surface is *resolvable by the host* — the row above it claimed the behaviour happens. Measured across every repository in the ecosystem: the only two hits for `applySubagentMemory` outside its definition are an export and a comment stating it has no caller. The row now says `resolvable` and dates the measurement. (#B-091)
- **The MCP uncarried-key warning named the project file even when it came from the personal one.** Reusing `parseMcpJson` for `~/.claude.json` — the right call, one parser for one format — inherited a message that hardcoded `.mcp.json`. Measured against the real file: the operator was told to look in `.mcp.json` for a `type` key that is not there. The source now travels into the diagnostic, which `parseMcpJson` already received and used for its errors. (#B-103)
- **Five disabled tests cited an owner that owns nothing.** `tests/unit/cron-scan.test.ts` pointed at a `theokit-test-suite-cleanup` followup plan as the owner of its debt. The name appears once in a commit subject, as a parenthetical on a change that happened — not a plan, and nothing that owns a category. A skip citing an owner reads as tracked debt, which is worse than reading as debt, because nobody goes looking for what is already owned. The pointer is gone and the file now carries what a reader needs: what the five cover, and the two ways out. (#B-109)
- **Portuguese in two versioned source files, and 116 unmarked exemptions in the detector itself.** `scripts/check-pack-entry-points-load.mjs` carried the same untranslated quotation the changelog did; it now reports what was said in English. The lexicon in `tests/lint/no-ptbr.test.ts` and the fixtures in `task-marker.test.ts` are legitimate — a gate must name what it detects, and a regression fixture must be in the language it regresses — but they carried the reason in prose the gate cannot read. Each line now carries the inline marker the rule requires, so the exemption is declared rather than tolerated. Released changelog sections stay untouched: they are immutable under Rule 6, Portuguese included. (#B-108)
- **The `[Unreleased]` section carried Portuguese that was about to freeze there.** A verbatim quotation sat in an entry, and `[Unreleased]` is the one mutable part of this file — at the next version cut it becomes immutable exactly as written. Reported in English rather than translated, because translating a quotation makes the quotation false. (#B-108)
- **The agents bundle ceiling moved 38 600 -> 38 700, by measurement.** Reading the operator's personal-scope MCP servers costs 52 bytes, attributed by building the barrel at the change's parent and at the change. Moving the module to a subpath was checked first, as the previous entry in that gate's history did, and rejected: it sits beside `loadMcpJson`, already in the barrel, and splitting a pair across two entry points is a worse surface than 52 bytes. Headroom stays at 72, not a kilobyte. (#B-103)
- **Two shipped features had no changeset, so no release would have published them.** `applySubagentMemory` and `loadPersonalMcpServers` were both committed, tested and green, and `.changeset/` held zero entries — the release is changesets-driven, so a release run would have bumped nothing and published neither. Measured against the consumer: the published `@theokit/agents@14.4.0` carries neither symbol while the local build carries both, under the same version number. (#B-107)
- **`themes/*.json` is documented as read by nobody, and why that is the right number.** After `keybindings.json` turned out to be read by the consumer rather than the package it was attributed to, the obvious move was to treat this as the same finding. This package has no colour at all — a grep for `color`, `chalk` or `theme` returns nothing, the one apparent hit being `ansi` inside `stateTransitionHistory` — and the toolkit reads no theme file either. Naming an owner would repeat the mistake the keybindings row already made. (#B-106)
- **`settings.json` → `env` is refused with a security reason, not left as an unexplained gap.** A variable is not inert: `NODE_OPTIONS=--require ./anything.js` is code execution, and `LD_PRELOAD`, `PATH` and `NODE_PATH` reach the same place. Everything this package spawns inherits the process environment — hook commands, and the stdio MCP servers whose `${VAR}` references already resolve against `process.env`. `.claude/settings.json` usually arrives with the clone, so applying its `env` would let a repository choose what runs inside every subprocess of anyone who opens it: the threat the `hooks` row is gated for, by a quieter door. (#B-105)
...
- **`CLAUDE.md` is documented as read when the host names it, which is when it is read.** The instruction tree's default file list is `['THEO.md', 'AGENTS.md', 'THEO.local.md', 'AGENTS.local.md']` and `instruction-tree.ts` states in its own docstring that `CLAUDE.md` is not in it. A consumer passing its own list gets it — `theocode` does. Found auditing the surfaces table row by row after `env` turned out to be the same shape: three rows claimed `read` for a mechanism the host has to invoke. (#B-102)
...
- **`pnpm run check:all` exits 0 again — three gates were measuring things this repository does not own.** They were stacked, and `run-p` reports only the first failure, so each repair revealed the next. (1) The aggregate built, then ran `typecheck` — which REBUILDS — in parallel with tasks reading that output, and `tsup` sets `clean: true`: the rebuild emptied `dist` mid-run. The symptom was "Could not find a declaration file" for a file 19056 bytes on disk, with the error count moving between runs (19, then 22) because it depended on timing, and `vitest` surfacing them as unhandled errors so the suite exited 1 with 7956 tests green. `typecheck:only` already existed and does not rebuild. (2) ESLint parsed eval fixtures under `.claude/`, an installed kit this repository does not version — invisible to CI, so the failure only ever appeared on a maintainer's machine. (3) The citation checker read `BACKLOG.md`, a registry that spans the ecosystem and is not versioned here; all 43 of its rotten citations named sibling repositories. Removing that noise revealed the number it was hiding: 1259 citations resolve and none are rotten. The same `dist` race also explains three `@theokit/agents` surface tests previously filed as flaky — they pack tarballs and read built output, and they pass now. (B-085, B-086)
...
## [@theokit/agents 14.2.0] - 2026-09-14

Released with no entry under `[Unreleased]`. Per-package detail is in each package's
own `CHANGELOG.md`.

## [@theokit/agents 14.1.0] - 2026-09-14

Released with no entry under `[Unreleased]`. Per-package detail is in each package's
own `CHANGELOG.md`.

## [@theokit/agents 14.0.0, create-theokit 3.0.0, theokit 0.66.0] - 2026-09-13
