...
- **M17 ACP stdio framing — `@theokit/agents`:** `encodeAcpMessage(msg)` and `AcpMessageDecoder` — the transport-agnostic core of the Agent Client Protocol (newline-delimited JSON) for talking to coding agents (Claude Code, Amp, Codex). The decoder buffers a message split across chunks, skips blank lines, and fails fast (typed error) on a corrupt frame. Pure — no subprocess, no Node API (the spawn lives in the adapter layer per G8 / SDK; `createACPTool` + `onPermissionRequest` gate are follow-ups). 6 tests; full suite 652 green; lint + tsc clean. (M17)
- **M13 Per-request skills resolution — `@theokit/agents`:** `resolveEnabledSkills(selection, ctx)` chooses the enabled skill set per request — `selection` is a static `string[]` OR a `(ctx) => string[]` resolver (sync/async) receiving the M7 run-context, so multi-tenant apps expose different skills to different users. `undefined` ⇒ the SDK enables every discovered skill. Fails fast when a resolver returns a non-array. (Confirmed the static `skills.enabled` filter already works — `compile-skills` maps `include` → the SDK's `enabled`; no bug there.) 5 tests; full suite 646 green; lint + tsc clean. Endpoint wiring of the resolver is a follow-up. (M13)
...
- **M16 MCP server manifest (generation) — `@theokit/agents`:** `buildMcpToolDescriptors(entry)` maps an agent's tools to MCP `tools/list` descriptors and `mcpServerInfo(entry)` produces the `initialize` server-info block (protocol `2024-11-05`). Pure data transform exposing a TheoKit agent's tools to external MCP clients over the app's own HTTP routes (ADR-0040 § D2) — no LLM, no runtime, no stdio transport (SDK-side). 4 tests; full suite 635 green; lint + tsc clean. Serving over `GET /mcp` + the JSON-RPC envelope are follow-ups. (M16)
- **M10 Tool-boundary hooks — `@theokit/agents`:** `createToolHooksPlugin({ beforeToolCall?, afterToolCall? })` — a plugin over the SDK's own `pre_tool_call` / `post_tool_call` hooks (mirrors `createHitlPlugin`, ADR-0040 § D2). `beforeToolCall` observes and may VETO a tool call (`{ block, message }`); `afterToolCall` observes the result. Registers only the hooks provided (inert when none). No LLM, no loop reimplementation. 5 tests; full suite 631 green; lint + tsc clean. The remaining processor lifecycle hooks (LLM request/response) are SDK-side; app-level wiring via `defineAgent` config is a follow-up. (M10)
- **M14 HITL approvals on `defineAgent` — `@theokit/agents`:** `defineAgent({ approvals: { <toolName>: { question, timeout?, onTimeout? } } })` gates a tool without the `@Agent` class + `@HumanInTheLoop` decorator. Compiles into the same `compiled.hitl` map the decorator path produces, so it reuses the already-proven endpoint HITL wiring (no new mount path). Fails fast at compile time when an approval names an undeclared tool. 3 tests; full suite 626 green; lint + tsc clean. `GET /approvals` listing + `errorStrategy` on `generateObject` (SDK) are follow-ups. (M14)
- **M15 A2A agent cards (generation) — `@theokit/agents`:** `buildAgentCard(entry, { baseUrl, description? })` produces an A2A-spec Agent Card from an `AgentManifestEntry` (name, absolute endpoint URL, `version`, `capabilities.streaming`, `defaultInput/OutputModes`, and each tool mapped to an A2A skill), plus `wellKnownCardPath(name)` → `/.well-known/<name>/agent-card.json`. Pure data transform, no LLM/runtime (G2). 5 tests; full suite 623 green; lint + tsc clean. Serving the card over an HTTP route (mount path), the `A2AAgent` client, and auth are follow-ups. (M15)
