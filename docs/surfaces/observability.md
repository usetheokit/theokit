...
| Request span | One `http.request` span per request, opened on `onRequest` and closed on `onResponse`/`onError` (`packages/theo/src/server/observability/middleware.ts:115`), continuing the caller's `traceparent` when there is one | Yes, **on the routes that run plugin hooks** — actions, `/api/*` and, since 2026-08-20, every agent endpoint (see the next row). Not on SSR page renders. See below |
...
| Agent run spans | `agent.run`, `agent.tool` and `agent.hitl` translated from the wire chunk stream (`packages/theo/src/server/agent/observe-agent-run.ts:134`) | Yes — `mountAgent` (`packages/theo/src/server/agent/mount-agent.ts:257`) and the thread route (`packages/theo/src/server/agent/build-agent-streamer.ts:102`) |
| **Delivery to a collector** | The exporter POSTs the serialized batch to the configured `ingestUrl` (`packages/theo/src/server/observability/adapters/theo-cloud.ts:115`); `scripts/probe-otlp-delivery.ts` drives `mountAgent` with the boot-resolved adapter and prints the marker to read back | Yes. **Measured 2026-09-23** against `otel/opentelemetry-collector-contrib`: an `agent.run` span produced by a run through `mountAgent` was quoted from the collector's own output. Until then every in-tree exercise substituted the transport (`_mockFetch`, six times), so the code that BUILDS a span was covered and the claim that one ARRIVES was not. The probe refuses to measure against an endpoint that answers below 400 to a path it does not serve — the first receiver this was pointed at did exactly that, and would have made "a POST left the process" read as "a span was accepted". **Re-confirmed 2026-09-24** with the instrument B-294 corrected the same day, because the 2026-09-23 run predated that fix: `exit 0` was unconditional then, since `flush()` awaits the POST for its side effect and discards the response, so it resolved the same whether the span was stored, refused or never sent. The conclusion survived on its readback rather than on the probe's word; the re-run earns it from an observed HTTP 200 — `agent.run` trace `29ff564cd144c5cfd6d9149dcdd2ebed`, its marker appearing once in 34,628 lines of collector output. **Neither run touches the built package**: the probe imports `packages/theo/src/server/agent/mount-agent.ts` through `tsx`, so the entry point is production's and the module tree is not the published one |
...
| **A run is one trace** | `observeAgentRun` mints one trace per run, pins the run span's id and names it as parent on every tool and pause span (`packages/theo/src/server/agent/observe-agent-run.ts:203`) | Yes. **Added 2026-08-20** (usetheokit/theokit#368) |
| **One request is one trace** | The `http.request` span and the run it contains both continue the caller's `traceparent` and both name the caller's span as parent, resolved by `extractW3CTraceContext` (`packages/theo/src/server/http/trace-context.ts:178`). Both served endpoints open their run spans through `observeServedRun` (`packages/theo/src/server/agent/observe-served-run.ts:80`), which is what stops them drifting again | Yes. **Added 2026-08-20** (usetheokit/theokit#385, #381). Before it, a request that ran an agent reached a collector as two disconnected traces — the caller's id was on the HTTP span as the `requestId` attribute, not as its `traceId` — and the thread route minted a third. The narrower resolver exists because `extractTraceIdFromRequest` always returns something and may return an `x-request-id` or a dashed UUID — correlation keys that are not trace ids |
| **Every served agent endpoint runs the plugin lifecycle** | One bracket — decorations, `onRequest` (which may short-circuit), the handler, `onError`, `onResponse` — applied by `serveThroughPluginLifecycle` (`packages/theo/src/server/http/plugin-lifecycle.ts:66`) to the plain run route, the HITL approve route and the six aux routes, in production and in dev. The aux dispatcher answers only after `matchAgentAuxRoute` has claimed the path (`packages/theo/src/server/agent/serve-aux-routes.ts:215`), which is what makes a bracket possible without converting a request to decide | Yes. **Added 2026-08-20** (usetheokit/theokit#405). Before it, the thread message and stream, MCP, agent card, approvals listing, run-stream reconnect and approve routes emitted **no `http.request` span at all** — an operator reading HTTP latency or error rate saw no traffic for endpoints that spend tokens and settle human decisions, and could not tell that from no traffic |
| **One agent is one series** | The `agent` attribute on `agent.run` / `agent.tool` / `agent.hitl` is the agent's scanned NAME on every route, because `observeServedRun` takes it as `agentName` rather than reusing the compile label (`packages/theo/src/server/agent/observe-served-run.ts:63`); `mountAgent` labels the spans with the same identity the access policy is judged under (`packages/theo/src/server/agent/mount-agent.ts:202`) | Yes. **Added 2026-08-20** (usetheokit/theokit#406). Before it the plain route exported the agent module's **absolute filesystem path** and the thread route the string `agent "chat"`, so one agent split into two series, the path form changed with every deploy, and the server's directory layout — on a developer machine, the user's account name — reached the telemetry backend on every span of every run |
...
| Token and cost attributes | Read from the producer's real shape, `usage.*` nested under the finish chunk's metadata (`packages/theo/src/server/agent/observe-agent-run.ts:170`) | Yes |
| **Model on the run span** | `gen_ai.request.model`, the OpenTelemetry GenAI semantic-conventions name (`packages/theo/src/server/agent/observe-agent-run.ts:143`), carrying the EFFECTIVE model — a per-run override beats the declared one, and an agent that declared none reports the default it ran on. It travels on the turn's `finish` metadata, resolved in the SDK adapter (`packages/agents/src/bridge/sdk-adapter.ts:411`) | Yes. **Added 2026-08-20** (B-019). Without it the token counts priced nothing, because price is per model — so "what did this run cost" was answerable only for providers that reported a cost themselves |
...
| Request logging | Emitted on every production route branch (`packages/theo/src/cli/commands/start/handlers.ts:123`) | Yes |
...
| Semantic conventions | Attribute names are the framework's own — `method`, `path`, `status`, `agent`, `tool` — so a backend cannot build standard views automatically. `gen_ai.request.model` is the exception. The agent module's path has a registry spelling too (`code.filepath`) and is deliberately **not** emitted: it was exported by accident until #406, and re-adding it is an opt-in decision, not a default |
