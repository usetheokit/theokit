...
| **Streaming on any shimmed target** | The Web shim accumulates every chunk into `responseChunks` and constructs the `Response` only inside `end()`. A streaming route is fully buffered before the first byte leaves | `packages/theo/src/adapters/web-shim.ts:157,190-210`; imported by bun, cloudflare, aws-lambda, netlify, deno-deploy and vercel (`packages/theo/src/adapters/bun.ts:52`, `packages/theo/src/adapters/cloudflare.ts:43`, `packages/theo/src/adapters/aws-lambda.ts:97`, `packages/theo/src/adapters/netlify.ts:38`, `packages/theo/src/adapters/deno-deploy.ts:33`, `packages/theo/src/adapters/vercel.ts:28`) |
