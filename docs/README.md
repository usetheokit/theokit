...
| `wiki/` | Authored procedures and decisions that ship with the product. `wiki/sops/` holds one runnable procedure per operation, under the shape `.claude/rules/sop-schema.md` defines and `mechanisms/gates/check_sop_structure.py` enforces — imperative steps, a decision tree whose every branch closes, and an escalation entry per way reality departs from the happy path. |
