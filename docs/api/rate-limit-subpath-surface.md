...
| `createDurableRateLimiterWeb` | promise | **Added by B-257 under ADR 0018.** It is the durable counterpart of `createRateLimiterWeb`: it accepts any `RateLimitStore` and awaits `incr`, where the sync facade refuses anything but the in-memory default because its closure cannot await. It is a PROMISE rather than internal because an application supplies the store — the framework ships none (`docs/adr/0017`), so reaching this symbol from outside the package is the whole point of it existing. It arrives through `theokit/server/rate-limit`, a first-class subpath declared at `packages/theo/package.json:78-81`, and NOT through the `theokit/server` umbrella whose removal at `0.x+2` the row above records. |
| `buildRateLimiter` | promise | **Added by B-262.** The generated deploy entry imports it BY NAME through this subpath — `packages/theo/src/adapters/deployed-rate-limit.ts:394` emits `import { buildRateLimiter } from '<prefix>theokit/server/rate-limit'` — and that entry runs in the CONSUMER's project, not in this repository. So the symbol is reached from outside the package on every deploy that declares a limit, which is the definition this file uses for a promise. It is one signature over both limiters and always async: the emitter used to branch on the config and the call site had to decide whether to `await`, which is where B-257's missing `await` lived. Removing this symbol breaks every entry built by a prior version that is redeployed against a newer one |
| `slidingCount` | internal | **Added by B-261.** Two consumers, both inside this package: `rate-limit.ts:7` and `rate-limit-durable.ts:1`. It reaches the subpath through `index.ts`'s `export *` — the accident this document exists to convert into a decision — and nothing outside the package calls it. A store author does not need it: they implement `incr` and return `previousCount`; the WEIGHTING is the limiter's job, and exporting the arithmetic would invite a caller to re-derive a refusal the limiter already makes. It is exported from its module rather than kept private because the two limiters must share ONE definition — B-261's own review measured what happens otherwise: the durable path held at 5 across a boundary while the sync path admitted 10, over the same store |
...
| `deriveKeyFromRequest` | internal | Zero consumers. The generated fragment resolves the address inline — `return addr(<expr>)` since B-027, which replaced the `?? 'unknown'` this row used to quote rather than calling it, so the Web-shaped mirror was superseded by the thing it was built for |
...
| `resolveClientIpFromRequest` | promise | **Five generated deploy entries import it by this exact specifier** — `cloudflare.ts:378`, `vercel.ts:53`, `netlify.ts:92`, `aws-lambda.ts:169` and `deno-deploy.ts:103` (that one `npm:`-prefixed). Removing it from the barrel does not degrade anything: every deployed entry declaring a rate limit stops LOADING, which is the strongest form a promise takes here. It reached the subpath for the first time in B-027 — the barrel listed four modules and `client-ip.js` was not one, so `rate-limited-guard.ts:60` reached it by relative path and the gap was invisible from inside the package. |
| `resolveClientIp` | internal | The NODE sibling, and it arrives on the subpath as a passenger of the `export *` that had to publish the Web one. Its only consumer is `rate-limit-per-route.ts:128`, inside this directory; no adapter emits it and none should — it falls back to `req.socket.remoteAddress`, which on a hosted platform is always the proxy, identical for every visitor. That is one shared bucket wearing a client's clothes, and `vercel.ts` records the measurement in its own comment. A consumer wanting the forwarded-header rule wants the row above. |
