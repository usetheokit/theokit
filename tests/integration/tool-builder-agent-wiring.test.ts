 ...
 *
 * Proves the identity-shape thesis end-to-end inside theokit: `tool()…build()` → `AgentBuilder.create().tool()`
...
import { AgentBuilder } from '../../packages/agents/src/bridge/agent-builder.js'
...
describe('tool() → AgentBuilder.create().tool() → compile (wiring)', () => {
  it('a tool()-built tool compiles into the agent exactly like defineAgentTool', () => {
    ...
    const builtAgent = AgentBuilder.create()
      .model('openrouter/openai/gpt-4o-mini')
      .tool(builtTool)
      .build()
    const legacyAgent = AgentBuilder.create()
      .model('openrouter/openai/gpt-4o-mini')
      .tool(legacyTool)
      .build()
