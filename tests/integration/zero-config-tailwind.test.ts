...
describe('T3.4 — zero-config-tailwind fixture', () => {
  it('fixture has @theokit/ui as a dep', () => {
  ...
  it('fixture uses @theokit/ui className conventions (.bg-primary etc)', () => {
    const page = readFileSync(resolve(FIXTURE, 'app/page.tsx'), 'utf-8')
    expect(page).toMatch(/bg-primary/)
  })

  it('detectPackage finds @theokit/ui from fixture root (when installed)', () => {
    // This depends on the workspace install having materialized.
    const detect = detectPackage('@theokit/ui', FIXTURE)
    // We're not asserting a strict true here because the workspace `@theokit/ui`
