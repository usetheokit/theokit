 ...
 * for `@theokit/ui/styles.css`. Browser only fetched CSS after JS
...
describe('injectStylesheets', () => {
  it('injects <link> for @theokit/ui/styles.css when package is present + isDev=true', () => {
    const result = injectStylesheets(BASE_HTML, {
      isDev: true,
      hasPackage: (name) => name === '@theokit/ui',
    })
    expect(result.injected.length).toBeGreaterThan(0)
    expect(result.html).toContain('<link rel="stylesheet" href="/@id/@theokit/ui/styles.css"')
    // Tag lands inside <head>, not in body
    const headSegment = result.html.split('</head>')[0] ?? ''
    expect(headSegment).toContain('@theokit/ui/styles.css')
  })
  ...
  it('does NOT inject when @theokit/ui is not in node_modules', () => {
    const result = injectStylesheets(BASE_HTML, {
      isDev: true,
      hasPackage: () => false,
    })
  ...
  it('idempotent: skips injection if href already present', () => {
    const htmlWithLink = `<!doctype html><html><head><link rel="stylesheet" href="/@id/@theokit/ui/styles.css" /></head><body></body></html>`
    ...
    expect(result.injected).toEqual([])
    // Only one occurrence
    const matches = result.html.match(/@theokit\/ui\/styles\.css/g) ?? []
