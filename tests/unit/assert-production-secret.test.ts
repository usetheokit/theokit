...
describe('assertProductionSecret (EC-2)', () => {
  ...
  it('throws in production when secret contains CHANGE_ME placeholder (even at 32+ chars)', () => {
    process.env.NODE_ENV = 'production'
    expect(() =>
      assertProductionSecret('CHANGE_ME_TO_RANDOM_32_PLUS_CHARS_FOR_REAL'),
    ).toThrow(/placeholder|CHANGE_ME/i)
  })

  it('throws in production when secret contains "demo-"', () => {
    process.env.NODE_ENV = 'production'
    expect(() => assertProductionSecret('demo-only-do-not-use-secret-32-chars')).toThrow(/placeholder|demo/i)
  })
