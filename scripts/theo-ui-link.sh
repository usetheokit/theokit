...
# with theokit-ui linked.
#
# Guards:
#   1. Sibling `../theokit-ui/` must exist.
#   2. EC-5 fix: `../theokit-ui/dist/vite-plugin.js` must exist (UI must be built
...
if [ ! -d "../theokit-ui" ]; then
  echo "Error: ../theokit-ui sibling checkout not found."
  echo "Clone theokit-ui to ../theokit-ui first, then re-run."
  exit 1
fi

# Guard 2 (EC-5): dist/ buildado
if [ ! -f "../theokit-ui/dist/vite-plugin.js" ]; then
  echo "Error: ../theokit-ui/dist/ not built (missing vite-plugin.js)."
  echo "Run \`pnpm --dir ../theokit-ui build\` first, then re-run \`pnpm theo-ui:link\`."
  exit 1
fi
...
echo "  Edits in ../theokit-ui/src/ reflect via HMR in your TheoKit dev server."
echo ""
