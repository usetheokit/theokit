...
function exerciseDep(dep, mod) {
  ...
  throw new Error(
    `[preflight-native-bindings] No exercise() defined for native dep '${dep}'. Add a branch in exerciseDep() to drive the binding's dlopen.`,
  )
}
...
export function findRebuildCwd(failingBindingPath, defaultCwd) {
  ...
  // eslint-disable-next-line sonarjs/slow-regex -- input is realpath bounded by filesystem max path; greedy quantifier safe
...
function runRebuild(dep, bindingPath) {
  ...
  // eslint-disable-next-line sonarjs/no-os-command-from-path -- build-time script spawned by repo developer; pnpm is the canonical package manager
