...
You are mid-implementation, iteration {ITERATION}. The user invoked `/implement v4-mainloop-reflective-runtime` to drive a TDD halt-loop over the implementation plan.

**Plan:** `.claude/knowledge-base/plans/v4-mainloop-reflective-runtime-plan.md`
**Implementation working contract:** `.claude/knowledge-base/implementations/v4-mainloop-reflective-runtime-implementation.md`
**Progress file:** `.claude/knowledge-base/implementations/.progress-v4-mainloop-reflective-runtime.json` (gitignored)
**SEPA agent file:** `.claude/agents/implement-v4-mainloop-reflective-runtime-2026-06-23/sepa.md` (Claude Code agent definition — frontmatter `name: implement-v4-mainloop-reflective-runtime-sepa`, body = role contract + verbatim plan/ADRs/edge-cases/audits/rules)
**SEPA paired knowledge skill:** `.claude/skills/implement-v4-mainloop-reflective-runtime-sepa-knowledge/SKILL.md` (Claude Code Skills-conformant; SEPA invokes via `Skill` tool for WebSearch refresh)
**SEPA per-iteration logs:** `.claude/knowledge-base/implementations/v4-mainloop-reflective-runtime/sepa-iterations/iteration-{N}-{phase}.md`

## SEPA invocation discipline (READ BEFORE Step 1)

Claude Code's agent registry is loaded at SESSION START — agent files written by `/implement` Step 2.5 mid-session may NOT be auto-discoverable by `subagent_type='implement-v4-mainloop-reflective-runtime-sepa'` until a fresh session starts. Two invocation paths are supported, in priority order:

1. **Primary (fresh session):** `Agent(subagent_type='implement-v4-mainloop-reflective-runtime-sepa', prompt=<iteration question>, description='SEPA iter {N} {phase}')`. The agent registry resolves `name:` from the SEPA file's frontmatter. Works only if the SEPA file was written BEFORE this session started.

2. **Fallback (mid-session):** `Agent(subagent_type='general-purpose', prompt='Read .claude/agents/implement-v4-mainloop-reflective-runtime-2026-06-23/sepa.md for your full role contract + plan/ADR/edge-case context. <iteration-specific question with MODE=TIGHT or MODE=VERBOSE>', description='SEPA iter {N} {phase}')`. The general-purpose agent reads the SEPA file at invocation time, so timing-of-write doesn't matter.
...
- `subagent_type`: `implement-v4-mainloop-reflective-runtime-sepa` (Primary) OR `general-purpose` (Fallback — pass SEPA file path in prompt)
- `prompt`: read `.claude/agents/implement-v4-mainloop-reflective-runtime-2026-06-23/sepa.md` for the role brief, then advise on task {T-ID} — recap what the plan declares for this task; surface gotchas, ADR-link expectations, files-to-edit verification, and TDD shape concerns. Return ONLY the structured advice format from the brief.
...
Append SEPA's response to `.claude/knowledge-base/implementations/v4-mainloop-reflective-runtime/sepa-iterations/iteration-{ITERATION}-pre-red.md` for audit trail (NOT `.claude/agents/` — that dir holds agent definitions only per Claude Code spec; logs go under knowledge-base/implementations/).
...
- `prompt`: read `.claude/agents/implement-v4-mainloop-reflective-runtime-2026-06-23/sepa-staff-engineer.md` for the role brief, then review the diff (captured via `git diff` against last commit). Spot SOLID/Clean Code/DRY violations, missed JSDoc cross-references, naming-convention drift, test-d completeness against ADR invariants.

Append response to `.claude/knowledge-base/implementations/v4-mainloop-reflective-runtime/sepa-iterations/iteration-{ITERATION}-post-green.md`.
...
- `subagent_type`: `general-purpose`
- `prompt`: read `.claude/agents/implement-v4-mainloop-reflective-runtime-2026-06-23/sepa-staff-engineer.md` for the role brief, then audit the staged diff (`git diff --cached`) + draft commit message against the task's DoD checkboxes from the plan. Verify: conventional-commit format, T-id reference, Wiring summary completeness, wiring triad sanity (pillar (a) callers are FUNCTIONAL not no-op stubs).

Append response to `.claude/knowledge-base/implementations/v4-mainloop-reflective-runtime/sepa-iterations/iteration-{ITERATION}-pre-commit.md`.
...
Append to `.claude/knowledge-base/implementations/.progress-v4-mainloop-reflective-runtime.json`:
...
- NEVER edit the implementation plan (`.claude/knowledge-base/plans/v4-mainloop-reflective-runtime-plan.md`) mid-iteration — the plan is the contract
...
- Mark the affected task as `blocked` with an explicit reason in `.progress-v4-mainloop-reflective-runtime.json`
