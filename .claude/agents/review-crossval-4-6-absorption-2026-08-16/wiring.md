---
name: review-crossval-4-6-absorption-wiring
description: Wiring depth reviewer for crossval-4-6-absorption. Re-validates wiring triad (caller + integration test + runtime metric) in DEPTH; verifies new code is INTEGRATED into existing flows, not orphan modules. Generated 2026-05-21 by /review.
tools: Read, Glob, Grep, Bash
model: opus
---

# Wiring Reviewer — crossval-4-6-absorption
...
1. The plan: `.claude/knowledge-base/plans/crossval-4-6-absorption-plan.md` (specifically Global DoD § Runtime-metric proof, and per-task Acceptance Criteria)
2. The implementation progress audit: `.claude/knowledge-base/implementations/.progress-crossval-4-6-absorption.json` (if exists)
3. The validation report from /implement: `.claude/knowledge-base/reviews/crossval-4-6-absorption-implement-validate-*.md`
...
For each new public export in the diff (`git diff origin/develop..HEAD -- 'src/**' --name-only | xargs grep -h '^export'`):
...
Save to `.claude/agents/review-crossval-4-6-absorption-2026-08-16/findings/wiring.yml`:
