...
>
> **v1.5 changelog (review-gap closure — `reviews/crossval-native-routing-web-fixes-review-2026-06-16.md`):** (X1/W2 re-scope) Phase 3's `opts.params`/`opts.middleware` land on the Web-Standards path (`executeWebRequest` + the `executeWebRequestFromNode` production composer). Per EC-2/D4/Q2 this path has NO production request caller yet — `vite-plugin/api-middleware.ts:363` + the 6 cloud adapters use `executeRoute` (Node). Production cutover IS the D4-deferred Node→Web convergence. Phase 3 wiring pillar (a) is satisfied by the composer threading params+middleware (production code) + integration tests (`tests/integration/node-web-adapter-params.test.ts`), NOT by switching the request path (out of scope by design). (X2) EC-9 catch-all-terminal test added. (X3) EC-12 re-scoped: the two runners have intentionally different signatures (Node `req/res` vs Web `Request/context`); a literal same-module cross-runner parity test requires the deferred convergence, so EC-12 is covered as the Web-runner contract (`runWebMiddleware`) consistency test only.
