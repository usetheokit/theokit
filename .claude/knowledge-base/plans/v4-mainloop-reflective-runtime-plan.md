...
- **LoopOutcome** — NEW value object describing one round's result: `{ finishReason, round, toolCalls, responseText }`. `finishReason` is derived in the bridge from the stream events (presence of `tool_result` without terminal `done` ⇒ `'tool-calls'`; terminal `done` ⇒ `'stop'`; `error` event ⇒ `'error'`; **any other round — empty stream / text-only / none of the above ⇒ `'stop'` (terminal DEFAULT; NEVER `'tool-calls'`, so a degenerate round can never spin the loop) — EC-1**).
...
RED:  test_resolve_maxiterations_boundaries() — resolveLoopStrategy('react',undefined) yields a FINITE default (not Infinity); resolveLoopStrategy('react',1).shouldContinue({finishReason:'tool-calls',round:1,...}) === false (maxIterations=1 ⇒ exactly one round) — EC-3
...
RED:  test_mainloop_empty_round_terminates_as_stop() — mock stream yields ZERO events (or text-only, no tool_result/done/error) on round1 → finishReason derives to 'stop' → loop terminates after 1 round (does NOT default to 'tool-calls' and spin to maxIterations) — EC-1
RED:  test_loop_budget_enforced_across_rounds() — per-round cost C, total budget < N×C across N rounds; assert the loop terminates mid-loop with BudgetExceededError once CUMULATIVE cost crosses budget (budget accumulates across rounds, not reset per round) — EC-4
...
| `@theokit/sdk` `Run.stream()` (LLM) | Degenerate round — empty stream / text-only, no `tool_result`/`done`/`error` | mock stream yields ZERO events (or text-only) on round 1 (`test_mainloop_empty_round_terminates_as_stop`) | `finishReason` derives to `'stop'` (terminal DEFAULT, EC-1) ⇒ loop terminates after 1 round — never defaults to `'tool-calls'`, never spins to `maxIterations` |
| `@theokit/sdk` `Run.stream()` (LLM) | Cumulative cost exceeds budget across rounds | per-round cost C, total budget < N×C (`test_loop_budget_enforced_across_rounds`) | loop terminates mid-loop with `BudgetExceededError` once cumulative cost crosses budget (accumulates across rounds, EC-4) |
