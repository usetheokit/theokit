...
- e5c4b19: `deriveActionKey` now accepts a salt as its second argument, so key derivation no longer depends on the secret it protects. Called without one it keeps deriving from the secret — existing ciphertext still decrypts — and warns once per process naming what is weak about it. Generate a random salt once, persist it beside the secret, and pass it.
...
- 2642c4d: `@theokit/http/css-resource` and `@theokit/http/server-inserted-html` now carry `@deprecated`, so a
