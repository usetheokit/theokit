...
class CatService {
  ...
  findById(id: string) {
    return this.cats.find((c) => c.id === Number(id)) ?? null
  }
  create(data: { name: string }) {
    const c = { id: this.nextId++, ...data }
    this.cats.push(c)
    return c
  }
  remove(id: string) {
    const i = this.cats.findIndex((c) => c.id === Number(id))
    if (i !== -1) this.cats.splice(i, 1)
  }
}
...
class ApiKeyGuard {
  canActivate(context: ExecutionContext) {
    ...
    return req.headers.get('x-api-key') === 'cats-secret'
  }
}
...
wp(CatsController, 'create', 0, 'body')
...
describe.skipIf(!isVitest)('TheoApp.create() — NestFactory/SpringBoot style', () => {
  let app: TheoApp
  let port: number
  ...
  it('GET /cats/secret com key → 200', async () => {
    const res = await fetch(`http://localhost:${port}/cats/secret`, {
      headers: { 'x-api-key': 'cats-secret' },
    })
    expect(res.status).toBe(200)
