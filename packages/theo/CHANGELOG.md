...
- 6a03209: `useNonce()` — an application component can read the request's CSP nonce
...
- Updated dependencies [80509ca]
  - @theokit/agents@15.0.1

## 0.72.1

### Patch Changes

- bc63e85: A middleware the file-scan runner cannot invoke names itself instead of vanishing.
...
- 799d406: Every deploy target's generated entry now resolves the caller's address from a source its own
  ...
  **`minor` rather than `patch`** because `theokit/server/rate-limit` publishes API it did not have:
  ...
  The comment that justified the old order is the reasoning error worth naming: _a 101 carries no
  document and no script, so the security baseline does not apply to it_. True of the security
  **headers**, a document concern. False of the limiter, a resource concern. One sentence was
  ...
  says the config _"reads as protection the deploy will not have"_.
...
- a4ead23: Every agent route of a freshly scaffolded, production-built app answered 500. Fixed
...
- 2642c4d: A server-rendered route now declares the chunks it needs, so the browser is told up front instead of
...
- Updated dependencies [f28ebd5]
- Updated dependencies [6c1a602]
- Updated dependencies [24e1fab]
- Updated dependencies [ef049e1]
- Updated dependencies [914e177]
  - @theokit/agents@14.0.0
