...
function generateRouteIndex(resourceName: string, fields: ResourceField[]): string {
  ...
  return [
    ...
    ``,
    `// TODO(policy): every route declares who may call it (ADR 0001). Generated as 'public'`,
    `// because the generator cannot know your access model — these four routes`,
    `// read and write a table, so decide before this ships. \`requireOwner(subject,`,
    `// row.ownerId)\` from 'theokit/server' is the per-record answer.`,
...
function generateRouteId(resourceName: string, fields: ResourceField[]): string {
  ...
  return [
    ...
    ``,
    `// TODO(policy): see the note in this resource's index route — 'public' here is a generated`,
    `// placeholder decision, not a recommendation for a table you own.`,
    `export const GET = route()`,
    `  .policy('public')`,
