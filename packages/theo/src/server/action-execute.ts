...
async function handleActionError(err: unknown, c: ActionErrorCtx): Promise<void> {
  // CR-004 parity: log + plugin onError + onResponse(inErrorPath) before
  // returning. Action errors were previously invisible to plugins.
  if (c.pluginRunner) {
    try {
      await c.pluginRunner.runOnError(c.buildPluginCtx(c.ctx), err)
    } catch {
      // onError handlers must never destabilize the response. Swallow
      // and continue to the response phase.
    }
  }

  if (err instanceof AuthRequiredError) {
    sendError(c.res, err.code, err.message, err.status, undefined, c.requestId)
  } else {
    sendError(
      c.res,
      'INTERNAL_ERROR',
  ...
  if (c.pluginRunner) {
    try {
      await c.pluginRunner.runOnResponse(c.buildPluginCtx(c.ctx), { inErrorPath: true })
    } catch {
      // Same containment as onError above.
    }
  }
}
