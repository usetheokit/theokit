...
import { deployedRuntimeConfigFragment } from './deployed-runtime-config.js'
...
export function renderCloudflareWorkerEntry(
  opts: {
    ...
    routes?: readonly { filePath: string; routePath: string; methods?: readonly string[] }[]
    /**
     * Specifier of the runtime-config module the build wrote beside this worker (#425).
     *
     * `config.plugins` and `config.serialization.transformer` carry functions, and a closure has
     * no literal — so unlike the security values above they are imported rather than baked.
     * Absent when the app declared neither, which keeps the emitted worker unchanged.
     */
