...
function renderNonApiBranch(
  ...
  return opts.ssrStreaming
    ? [
        ...
        `      // document before this code runs. Rendering SSR for those would answer a text file with`,
        `      // HTML. The discriminator is an extension on the LAST segment: \`/users/42\` has none and is`,
        `      // a route, \`/robots.txt\` has one and is a file.`,
        ...
        `        return withSecurityHeaders(await staticAssets.fetch(request), SECURITY_HEADERS)`,
        `      }`,
        ``,
