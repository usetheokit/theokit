...
function awsLambdaHandlerFragment(): string[] {
  return [
    ...
    `  const response = withSecurityHeaders(await routeRequest(event, path), SECURITY_HEADERS)`,
...
export async function buildAwsLambda(
  ...
  const entry = renderAwsLambdaEntry({ securityHeaders: config.security?.headers })
  const write =
    deps.writeEntry ??
    ((p, c) => {
      writeFileSync(p, c)
