...
export function renderVercelFunctionEntry(): string {
  return [
    ...
    `  }))`,
    ``,
    `  // \`outHeaders\`, not \`headers\`: the request-side \`const headers\` above is`,
    `  // in the same function scope, and re-declaring it made this whole module a`,
    `  // SyntaxError -- every Vercel build since #382 emitted a function that could`,
    `  // not be loaded (usetheokit/theokit#411).`,
    `  const outHeaders = {}`,
