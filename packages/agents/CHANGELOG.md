...
- 309c717: The agent can declare the work it is judged on.
...
- 8f35df1: `settingSources: { user: true }` now reads the operator's roots instead of changing nothing.
  ...
  package's README is explicit that _"the barrel is not the API"_, and the bundle gate measured the
...
- 22a36da: `withPreCompaction` runs your work — and awaits it — before a transcript is rewritten.
