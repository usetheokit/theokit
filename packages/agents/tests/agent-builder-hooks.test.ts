...
const pluginsOf = (def: unknown): Array<Record<string, unknown>> =>
  ((compileAgentDefinition(def as never) as Record<string, unknown>).plugins ??
    []) as Array<Record<string, unknown>>

const registered = (plugin: Record<string, unknown>): string[] => {
  const seen: string[] = []
  ;(plugin.register as (c: unknown) => void)({ on: (n: string) => void seen.push(n) })
  return seen
}

describe('agent().hooks()', () => {
  it('reaches the compiled options as a code plugin the SDK will accept', () => {
    const a = agent().input(z.object({ m: z.string() })).model('x')
      .hooks({ pre_tool_call: () => undefined }).build()
  ...
  it('registers every function-valued hook under its own event name', () => {
    const a = agent().input(z.object({ m: z.string() })).model('x')
      ...
      .build()
    expect(registered(pluginsOf(a)[0]!).sort()).toEqual(['on_session_start', 'pre_tool_call'])
  })

  it('COMPOSES with plugins() — hooks and plugins are additive, not exclusive', () => {
    const mine = { name: 'mine', kind: 'general', register: () => {} }
    const a = agent().input(z.object({ m: z.string() })).model('x')
      .plugins([mine]).hooks({ pre_tool_call: () => undefined }).build()
  ...
  it('a later hooks() call REPLACES the map rather than accumulating', () => {
    const a = agent().input(z.object({ m: z.string() })).model('x')
      .hooks({ pre_tool_call: () => undefined }).hooks({ on_session_start: () => {} }).build()
  ...
  it('adds no plugin when no hook is declared', () => {
    const a = agent().input(z.object({ m: z.string() })).model('x').hooks({}).build()
...
describe('defineAgent({ hooks })', () => {
  ...
  it('leaves plugins untouched when no hooks are declared', () => {
    const mine = { name: 'mine', kind: 'general', register: () => {} }
    expect(pluginsOf(defineAgent({ model: 'x', plugins: [mine] })).map((p) => p.name)).toEqual(['mine'])
  })
})
