/**
 * M31 Phase 2 — `AgentBuilder.create()` builder gains `.guardrail(s)` / `.approval(s)` / `.skills`. Each sets the
...
describe('AgentBuilder.create() builder — M31 extensions', () => {
  it('.guardrail() appends and .guardrails() replaces', () => {
    const compiled = compileAgentDefinition(
      AgentBuilder.create().model('m').guardrail(passGuard).build(),
    )
  ...
  it('.approval() merges a HITL gate keyed by tool name', () => {
    const compiled = compileAgentDefinition(
      AgentBuilder.create()
        .model('m')
        .tool(namedTool('write'))
        .approval('write', { question: 'ok?' })
        .build(),
    )
  ...
  it('.skills() sets a static skill selection', () => {
    const compiled = compileAgentDefinition(
      AgentBuilder.create().model('m').skills(['fs', 'web']).build(),
    )
