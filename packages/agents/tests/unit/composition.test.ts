...
describe('applyDecorators — decorator composition', () => {
  it('test_compose_agent_preset', () => {
    const ProductionAgent = (name: string, route: string) =>
      applyDecorators(
  ...
  it('test_compose_reused_across_agents', () => {
    const ProductionAgent = (name: string, route: string) =>
      applyDecorators(Agent({ name, route }), UseGuards(AuthGuard))
  ...
  it('test_compose_method_level_decorators', () => {
    const SecureTool = (name: string, desc: string) =>
      applyDecorators(Tool({ name, description: desc, input: z.object({}) }), Trace(true))
  ...
  it('test_compose_can_be_extended', () => {
    const BaseAgent = (name: string, route: string) =>
      applyDecorators(Agent({ name, route }), UseGuards(AuthGuard))
...
describe('@Mixin — capability composition', () => {
  ...
  it('test_mixin_multiple_applications_accumulate', () => {
    /* eslint-disable @typescript-eslint/no-extraneous-class -- @Mixin fixtures: identity is all that matters */
    class A {}
    class B {}
    class C {}
