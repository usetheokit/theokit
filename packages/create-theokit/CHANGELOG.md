...
- The scaffold's pins now match what this release publishes, so a project created from it
  ...
  - theokit: ^0.68.0 -> ^0.69.0

## 3.0.1

### Patch Changes

- 2feb395: Bump this package whenever the release corrects the framework pin in its scaffold template, so the
  ...
  skipped this package and said why in its own words: _"create-theokit is not being published because
  version 3.0.0 is already published on npm"_. Nothing bumped it, because the pin lives in a template
...
- f28ebd5: A scaffolded project now pins `@theokit/sdk@^5.0.0`, the line that actually carries the parity.
