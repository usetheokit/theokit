...
const failingCheck = async () => ({ name: 'redis', healthy: false, message: 'connection refused' })

describe('Real E2E — TheoKit Full-Stack', () => {
  let app: TheoApp
  let port: number
  let baseUrl: string
  ...
  it('GET /__theo/ready returns ready when checks pass', async () => {
    ...
    expect(res.status).toBe(200)
    const body = await res.json() as { status: string; checks: { name: string; healthy: boolean }[] }
  ...
  it('TypedClient.post() sends body and returns typed response', async () => {
    const client = createTypedClient<Record<string, { response: unknown }>>(`${baseUrl}`)
    const task = await client.post('/api/tasks', { title: 'Client Task', priority: 'low' }) as { id: number; title: string }
  ...
  it('TypedClient throws TypedClientError on 422', async () => {
    const client = createTypedClient<Record<string, { response: unknown }>>(`${baseUrl}`)
    await expect(
      client.post('/api/tasks', { title: 'ab' }),
    ).rejects.toThrow(TypedClientError)
  })
