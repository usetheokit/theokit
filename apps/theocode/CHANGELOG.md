...
### Fixed
- **A malformed `settings.json` no longer takes the terminal down, and says so on every boot.** Once a directory is trusted its project config IS read, and `resolveEffectiveConfig` throws on a broken file — the session died with a stack trace before the first frame. The warning that did exist lived inside the branch that runs when trust is GRANTED, so later sessions got nothing at all. The session now starts on the CONFINED posture (`read-only`, approval on request) rather than on defaults — the operator expressed a preference that could not be read, and granting write authority on the strength of a file we just failed to parse is the opposite of what the failure warrants — and the reason is on screen beside the credential notice, which exists for the same argument. The message also names `settings.json` instead of `config.toml`, the format it replaced. (#827)
...
- **An enterprise `managed-settings.json` is read, and nothing can override it.** The reference defines it as settings a user cannot override; this product read no such file, so an organisation deploying a policy — a forced model, a denied tool, a required hook — had it silently dropped while the same policy held in the tool it was written for. The failure direction was *permit*, which is what makes it an ignored control rather than a missing feature. It now sits above every layer including the operator's own `-c` flag, and above the security floor: a managed policy binds the person at the keyboard, which is the entire point of one. The same platform paths the reference uses, so an administrator deploys one file rather than two. (#737)
- **Every documented `settings.json` key now carries a decision, not a silence.** Reporting a key as "not implemented here" says the code does not act on it and never says whether that is a refusal or an omission — so an author could not tell whether to stop writing the key or to wait for it. `model` and `cleanupPeriodDays` are honoured: the first already was, the second now reaches the session collector as `--max-age-days` instead of being dropped for the default 30. `statusLine` and `autoMemoryEnabled` are refused with a reason — the first executes a shell command written in a file the repository ships, the second toggles a feature this product does not have. (#736)
- **`doctor` stopped calling the `permissions` block inert.** It had been listed under "not implemented here" on the honest grounds that nothing carried the rules to an engine. That became false when the engine was wired, and a diagnostic that tells an operator a control is inert while it is refusing their reads is worse than the gap it used to describe. Entries the grammar cannot render are still named, because a block taking effect says nothing about a line that never became a rule. (#736)
- **A `deny` rule now wins over an `allow` written in another layer.** `PermissionEngine` is first-match-wins — measured against the SDK, `[allow Read, deny Read]` evaluates `allow` and the reverse evaluates `deny` — and the layers were concatenated in whatever order they were read. Measured on the built binary with a project `deny: ["Read(./off-limits.txt)"]` and an operator `allow` list that happened to contain `Read`: the file's contents came back. Denies are now placed ahead of allows, which is right in both directions — a repository cannot grant itself what the operator refused, and a broad convenience allow cannot disarm a refusal a repository wrote about its own files. (#736)
- **A tool nobody wrote a rule about is no longer dead.** Enforcing the `permissions` block wired an engine whose verdict for an unmatched tool is `ask`, and the SDK turns `ask` into a hard block when nothing answers it — so the first `permissions` block an operator wrote killed every tool they had not enumerated. Measured with one rule allowing `Read(./src/**)`: `skill_read` came back "requires approval" for a skill that loads, and adding it to `allow` made the same call succeed; delegation died identically. The gate that answers `ask` is now mandatory and derived from `approval_policy` — `never` runs it, anything else refuses with a message naming the tool and the two ways to change the outcome. (#826, #828)
- **A skill shipped inside a plugin bundle is readable.** `.claude/plugins/<bundle>/skills/<name>/SKILL.md` was walked by two files that each cited the other as the reason it was covered, and neither made the skill loadable: `skill_read("bundled")` answered `not found` for a file sitting beside one it did find. The bundle roots now join the list `skill_read` resolves against, `skills-on-disk` counts them under their own root instead of omitting them, and `foreign-surfaces` carries a `plugins` row where it carried none. (#826)
- **A subagent that declares `memory: project` now starts with its memory.** The framework read the file and folded it into the definition, and the definition was then dropped: the member was built from the model, the tools and the sandbox, with no prompt at all. Nothing reported a difference, because the SDK reads `.claude/agents/<name>.md` itself and fell back to the prompt in the FILE — so the subagent answered normally while running without the memory. Measured beside Claude Code on byte-identical configuration: asked what it knew without reading anything, Claude Code returned the canary and this product returned only the definition's own instructions. (#825)
